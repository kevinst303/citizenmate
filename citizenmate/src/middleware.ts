...
const PROTECTED_API_ROUTES = ["/api/admin", "/api/checkout"];
...
export const config = {
  matcher: [
    '/((?!_next/static|_next/image|favicon.ico|sitemap.xml|robots.txt|sw\\.js|manifest\\.json|icons/|api/webhooks|.*\\.svg$|.*\\.png$|.*\\.webp$|.*\\.jpg$|.*\\.jpeg$|.*\\.gif$|.*\\.ico$|.*\\.woff2?$|.*\\.ttf$|.*\\.eot$|.*\\.json$|.*\\.xml$).*)',
  ],
};
