...
export function QuizCTA({
  ...
  text = 'Don\'t wait until the last minute. Familiarize yourself with the actual test format using our hyper-realistic simulator.',
}: QuizCtaProps) {
  ...
  return (
    <div className="my-10 sm:my-16 group/card card-conseil-popular action-card-shine grid grid-cols-1 md:grid-cols-5 gap-6 sm:gap-8 md:gap-12 items-center p-6 sm:p-10 lg:p-12 not-prose relative overflow-hidden rounded-2xl sm:rounded-[32px] border border-white/10 shadow-[0_20px_40px_-15px_rgba(0,109,119,0.3)] transition-all duration-500 hover:shadow-[0_30px_60px_-15px_rgba(0,109,119,0.5)] hover:-translate-y-1">
      ...
      <div className="space-y-4 sm:space-y-6 md:col-span-3 relative z-10 text-center sm:text-left">
        ...
        <div className="pt-2 sm:pt-4">
          <div className="relative inline-block w-full sm:w-auto group/btn">
            ...
            <Link href={getUrl('/practice')} className="no-underline block relative">
              <span className="inline-flex w-full sm:w-auto bg-white text-cm-teal hover:bg-slate-50 hover:text-cm-teal-dark shadow-[0_8px_20px_rgba(0,0,0,0.15)] hover:shadow-[0_12px_30px_rgba(0,0,0,0.25)] text-sm sm:text-lg h-12 sm:h-14 px-6 sm:px-10 rounded-full font-bold transition-all duration-300 hover:scale-[1.02] sm:hover:scale-[1.03] active:scale-[0.98] sm:active:scale-[0.97] border border-white/60 items-center justify-center gap-2 group-hover/btn:border-white">
      ...
      <div className="hidden md:block md:col-span-2 relative z-10">
        <div className="relative w-full aspect-square max-w-[280px] mx-auto transition-transform duration-700 ease-out transform-gpu group-hover/card:scale-105">
          {/* Main Mockup Card */}
