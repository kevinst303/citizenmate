          ...
          <div className="flex items-center justify-between h-14 sm:h-16">
            ...
            <div className="flex items-center gap-2 text-sm font-medium text-cm-slate-600">
              <span className="font-heading">
                ...
                {t("quiz.of", "of")} {state.test.questions.length}
              </span>
            </div>
          ...
          >
            ...
            >
              <h3 className="text-xl font-heading font-bold text-cm-slate-900 mb-2">
                {t("quiz.submit_title", "Submit your test?")}
              </h3>

              {unansweredCount > 0 ? (
                <p className="text-cm-slate-600 mb-6">
                  {t("quiz.you_have_prefix", "You have ")}
                  <span className="font-bold text-amber-600">
                    {unansweredCount}{t("quiz.unanswered_count", " unanswered ")}
                    {unansweredCount === 1 ? t("quiz.question_singular", "question") : t("quiz.question_plural", "questions")}
                  </span>
                  {t("quiz.marked_incorrect", ". Unanswered questions will be marked as incorrect.")}
                </p>
              ) : (
                <p className="text-cm-slate-600 mb-6">
                  {t("quiz.answered_all_prefix", "You've answered all ")}{state.test.questions.length}{t("quiz.ready_for_results", " questions. Ready to see your results?")}
                </p>
              )}
              ...
              <div className="flex items-center gap-3 text-sm text-cm-slate-500 mb-6 bg-cm-slate-50 rounded-xl p-3">
                <span className="flex items-center gap-1.5">
                  ...
                  {answeredCount}{t("quiz.answered_label", " answered")}
                </span>
                <span className="text-cm-slate-300">·</span>
                <span className="flex items-center gap-1.5">
                  <Flag className="w-3.5 h-3.5 text-amber-500" />
                  {state.flaggedQuestions.size}{t("quiz.flagged_label", " flagged")}
                </span>
                <span className="text-cm-slate-300">·</span>
                <span className="flex items-center gap-1.5">
                  <XCircle className="w-3.5 h-3.5 text-red-400" />
                  {unansweredCount}{t("quiz.unanswered_label", " unanswered")}
                </span>
              </div>
